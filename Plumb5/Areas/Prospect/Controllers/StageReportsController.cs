...
{
    [Area("Prospect")]
    public class StageReportsController : BaseController
    {
        public StageReportsController(IConfiguration _configuration) : base(_configuration)
        { }

        //
        // GET: /Prospect/StageReports/
        ...
        [HttpPost]
        public async Task<JsonResult> GetMaxCount([FromBody] StageReportsDto_GetMaxCount commonDetails)
        {
            ...
            using (var objBL = DLStageReports.GetDLStageReports(commonDetails.accountId, SQLProvider))
            {
        ...
        [HttpPost]
        public async Task<ActionResult> GetReport([FromBody] StageReportsDto_GetReport commonDetails)
        {
            LoginInfo? user = JsonConvert.DeserializeObject<LoginInfo>(HttpContext.Session.GetString("UserInfo"));
            ...
            using (SourceStageReportModel sourceStagemodel = new SourceStageReportModel(SQLProvider))
                sourceReport = await sourceStagemodel.GetStageReport(commonDetails.accountId, FromDateTime, ToDateTime, commonDetails.Offset, commonDetails.FetchNext, UseridList, commonDetails.IsCreatedDate);
        ...
        [HttpPost]
        public async Task<ActionResult> ExportStageReport([FromBody] StageReportsDto_ExportStageReport commonDetails)
        {
            LoginInfo? user = JsonConvert.DeserializeObject<LoginInfo>(HttpContext.Session.GetString("UserInfo"));
            ...
            using (SourceStageReportModel sourceStagemodel = new SourceStageReportModel(SQLProvider))
                DataSet = await sourceStagemodel.GetStageReport(commonDetails.AccountId, FromDateTimes, ToDateTime, commonDetails.OffSet, commonDetails.FetchNext, UseridList, IsCreatedDate);
            ...
            string FileName = "LMSStageReport_" + DateTime.Now.ToString("ddMMyyyyHHmmssfff") + "." + commonDetails.FileType;
