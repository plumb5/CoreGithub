(function (root, factory) {
    ...
    QueryBuilder.DEFAULTS = {
        ...
        lang: {
            "add_rule1": 'Add Columns',
            "add_rule": 'Add Conditions',
            "add_group": 'Add group',
            "delete_rule": 'Delete',
            "delete_group": 'Delete',
        ...
        ////{ type: 'is_not_null', nb_inputs: 0, multiple: false, apply_to: ['string', 'number', 'datetime', 'boolean'] }
            { type: 'select', accept_values: 0, apply_to: ['string'] },
        ...
        icons: {
            ...
            add_rule1: 'glyphicon glyphicon-plus',
   ...
   <dl id="'+ group_id + '" class="rules-group-container"> \
     <dt class="rules-group-header"> \
       <div class="btn-group pull-right group-actions"> \
         ...
         '<button type="button" class="btn btn-xs btn-success" data-add="group" style="display:none;"> \
    ...
    <div class="btn-group group-conditions" style="display:none"> \
      '+ this.getGroupConditions(group_id) + ' \
    </div> \
  '+ (this.settings.display_errors ?
           '<div class="error-container"  style="display:none;" data-toggle="tooltip" data-placement="right"><i style="top:16px;" class="' + this.icons.error + '"></i></div>'
         : '') + '\
  </dt><span style=\"font-weight:bold;padding-left: 3px;\"> Select column </span> \
</br><dd class=rules-group-body style=\"padding-top: 4px;\"> \
...
<dd class=rules-group-body> \
    ...
    QueryBuilder.prototype.getRuleTemplate = function (rule_id) {
        var h;
        if (rule_id.indexOf("builder1_rule_") != -1) {
            h = '\
  ...
  <div class="rule-header"> \
    <div class="btn-group pull-right rule-actions"> \
      <button type="button" class="btn btn-xs btn-danger" data-delete="rule"> \
        ...
        '+ (this.settings.display_errors ?
            '<div class="error-container"><i style="top:16px;"  class="' + this.icons.error + '"></i></div>'
            : '') + '\
...
<li id="'+ rule_id + '" class="rule-container"> \
  ...
  <div class="rule-value-container"></div> \
    ...
    QueryBuilder.prototype.getRuleFilterSelect = function (rule_id) {
        ...
        var h = '<span style=\'width:22%\'><select style=\'width:110px;\' name="' + rule_id + '_filter">';
    ...
    QueryBuilder.prototype.getRuleOperatorSelect = function (rule_id, operators) {
        var h = '<span style=\'width:22%\'><select  style=\'width:110px;\'  name="' + rule_id + '_operator">';
    ...
    QueryBuilder.prototype.getRuleInput = function ($rule, filter, value_id) {
        ...
        if (typeof filter.input === 'function') {
            ...
            switch (filter.input) {
                ...
                case 'select':
                    h += '<select style=\'height:35px;\' name="' + name + '"' + (filter.multiple ? ' multiple' : '') + '>';
                    iterateOptions(filter.values, function (key, val) {
                ...
                case 'textarea':
                    h += '<textarea style=\'height:100px;\'name="' + name + '"';
                ...
                default:
                    switch (filter.internalType) {
                        case 'number':
                            h += '<input style=\'height:35px;\'  type="number" name="' + name + '"';
                        ...
                        default:
                            h += '<input style=\'height:35px;\' type="text" name="' + name + '"';
                            if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
    ...
    $.fn.queryBuilder.defaults.set({
        sqlOperators: {
            ...
            in: { op: 'IN(?)', fn: function (v) { var k = ''; var a = ''; var arr = []; if (v.indexOf(",") != -1) { arr = v.split(","); for (var i = 0; i < arr.length; i++) { if (i === 0) { k += arr[i] + "',"; } else if (i == arr.length - 1) { k += "'" + arr[i] + ","; } else { k += "'" + arr[i] + "',"; } } } else { k = v; } if (k.indexOf(",") != -1) { var n = k.lastIndexOf(","); a = k.substring(0, n) } else { a = k; } return a; } },
            not_in: { op: 'NOT IN(?)', fn: function (v) { var k = ''; var a = ''; var arr = []; if (v.indexOf(",") != -1) { arr = v.split(","); for (var i = 0; i < arr.length; i++) { if (i === 0) { k += arr[i] + "',"; } else if (i == arr.length - 1) { k += "'" + arr[i] + ","; } else { k += "'" + arr[i] + "',"; } } } else { k = v; } if (k.indexOf(",") != -1) { var n = k.lastIndexOf(","); a = k.substring(0, n); } else { a = k; } return a; } },
            less: '< ?',
            less_or_equal: '<= ?',
            greater: '> ?',
            greater_or_equal: '>= ?',
