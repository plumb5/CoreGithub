...
var ruleConditions = {
    ...
    IsBusinessOrIndividualMember: null, IsOfflineOrOnlinePurchase: null, LastPurchaseIntervalCondition: 0, LastPurchaseIntervalRange1: 0, LastPurchaseIntervalRange2: 0,
...
ValidationOfAudience = function () {
    if ($("#ui_chkSegment").is(":checked") && $("input[name='BelongsGroup']:checked").val() && $("#ui_txtGroups_values").children().length == 0) {
        ShowErrorMessage("Display Rules -> Audience -> Please enter the groups");
        $("#ui_txtGroups").focus();
        return false;
    }
    return true;
...
ValidateBehavior = function () {
    if ($("#ui_chkBehavioralScore").is(":checked")) {
        if ($("#ui_VisitorScore1").val().length == 0) {
            ShowErrorMessage("Display Rules -> Behavior -> Please enter behavioral score");
            ...
            ShowErrorMessage("Display Rules -> Behavior -> behavioral score range is not correctly");
            $("#ui_VisitorScore2").focus();
            return false;
        }
    }
    ...
    if ($("#ui_chkOnPageUrl").is(":checked") && $("#ui_txtPageUrl_values").children().length == 0) {
        ShowErrorMessage("Display Rules -> Behavior -> Please enter the screen name");
        $("#ui_txtPageUrl").focus();
        return false;
    }
    if ($("#ui_chkOnQueryString").is(":checked") && $("#ui_txtQueryString").val().length == 0) {
        ShowErrorMessage("Display Rules -> Behavior -> Please enter query strings");
        $("#ui_txtQueryString").focus();
        return false;
    }
    ...
    if ($("#ui_chkAlreadyVisitedPages").is(":checked") && $("#ui_txtAlreadyVisitedPageUrls_values").children().length == 0) {
        ShowErrorMessage("Display Rules -> Behavior -> Please enter the screen name");
        $("#ui_txtAlreadyVisitedPageUrls").focus();
        return false;
    }
    if ($("#ui_chkTimeSpentInSite").is(":checked") && $("#ui_txtTimeSpentInSite").val().length == 0) {
        ShowErrorMessage("Display Rules -> Behavior -> Please enter over all time spent in app");
...
ValidateInteraction = function () {
    if ($("#ui_chkRecentClickedButton").is(":checked") && $("#ui_txtRecentClickButton_values").children().length == 0) {
        ShowErrorMessage("Display Rules -> Interaction -> Please enter recent Button tag names");
        $("#ui_txtRecentClickButton").focus();
        return false;
    }
    if ($("#ui_chkClickedButton").is(":checked") && $("#ui_txtClickButton_values").children().length == 0) {
        ShowErrorMessage("Display Rules -> Interaction -> Please enter specific Button tag names");
        $("#ui_txtClickButton").focus();
        return false;
    }
    if ($("#ui_chkClickedPriceRange").is(":checked")) {
        if ($("#ui_txtPriceRangeProducts_values").children().length == 0) {
            ShowErrorMessage("Display Rules -> Interaction -> Please enter specific price range tag");
            $("#ui_txtPriceRangeProducts").focus();
            return false;
        }
    }
    if ($("#ui_chkRespondedForm").is(":checked") && $("#ui_ddlRespondedFroms").val() == 0) {
        ShowErrorMessage("Display Rules -> Interaction -> Please select the form");
...
ValidateInteractionEvent = function () {
    if ($("#ui_chkFormImpression").is(":checked")) {
        if ($("#ui_ddlFormImpression").val() == 0) {
            ShowErrorMessage("Display Rules -> Interaction Event -> Please select the form");
            $("#ui_ddlFormImpression").focus();
            return false;
        }
    ...
    if ($("#ui_chkFormCloseEvent").is(":checked")) {
        if ($("#ui_ddlFormCloseEvent").val() == 0) {
            ShowErrorMessage("Display Rules -> Interaction Event -> Please select the form");
            $("#ui_ddlFormCloseEvent").focus();
            return false;
        }
    ...
    if ($("#ui_chkResponseCountEvent").is(":checked")) {
        if ($("#ui_ddlResponseCountEvent").val() == 0) {
            ShowErrorMessage("Display Rules -> Interaction Event -> Please select the form");
            $("#ui_ddlResponseCountEvent").focus();
            return false;
        }
    ...
    if ($("#ui_chkShowFormAtNTime").is(":checked") && $("#ui_txtShowFormAtNTime").val().length == 0) {
        ShowErrorMessage("Display Rules -> Interaction Event -> Please enter value for 'Show this form only Nth Time' condition");
...
ValidateProfile = function () {
    ...
    if ($("#ui_chkProspectLoyaltyIs").is(":checked")) {
        if ($("#ui_txtProspectLoyalty1").val().length == 0) {
            ShowErrorMessage("Display Rules -> Profile -> Please enter loyalty score");
    ...
    if ($("#ui_chkRFMSScoreIs").is(":checked")) {
        if ($("#ui_txtRFMSScore1").val().length == 0) {
            ShowErrorMessage("Display Rules -> Profile -> Please enter RFMS score");
...
$("#ui_chkReportMail, #ui_chkReportSMS, #ui_chkSendMailOut, #ui_chkSalesPerson").click(function () {
...
function AppendSelected(appendTo, data, fieldId) {
    ...
    mainDiv.id = fieldId + "_" + data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/'/g, "_").replace(/\./g, "-");
    mainDiv.className = "vrAutocomplete";
    mainDiv.setAttribute("value", data.item.value);
    mainDiv.setAttribute("identifier", data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/'/g, "_").replace(/\./g, "-"));
    ...
    removeDiv.setAttribute("onclick", "RemoveData('" + fieldId + "_" + data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/'/g, "_").replace(/\./g, "-") + "');");
    ...
    $("#" + appendTo).children().each(function () {
        if (data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/'/g, "_").replace(/\./g, "-") == $(this).attr("identifier")) {
...
//// For Screen
function AppendSelectedScreen(appendTo, data, fieldId) {
    var mainDiv = document.createElement("div");
    mainDiv.id = fieldId + "_" + data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/\./g, "-");
    mainDiv.className = "vrAutocomplete";
    mainDiv.setAttribute("value", data.item.value);
    mainDiv.setAttribute("identifier", data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/\./g, "-"));
    ...
    var removeDiv = document.createElement("div");
    removeDiv.className = "vmAutocomplete";
    removeDiv.setAttribute("onclick", "RemoveDataScreen('" + fieldId + "_" + data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/\./g, "-") + "');");
    ...
    $("#" + appendTo).children().each(function () {
        if (data.item.value.replace(/ /g, "_").replace(/,/g, "_").replace(/\./g, "-") == $(this).attr("identifier")) {
            isElementIsNotAdded = false;
            return false;
        }
    });
...
function RemoveDataScreen(data) {
    data = $.trim(data);
    $("#" + data).remove();
}
        ...
        success: function (fields) {
            ...
            if (fields.length > 0) {
                $.each(fields, function (i) {
                    ...
                    else if ($(this)[0].SubFields != null && $(this)[0].SubFields.length > 0) {
                        var subFields;
...
BindBehaviorData = function () {
    ...
    if (ruleConditions.PageUrl != null && ruleConditions.PageUrl.length > 0) {
        needToOpendDiv = true;
        var clickedPage = ruleConditions.PageUrl.split(",");
        for (var i = 0; i < clickedPage.length; i++) {
            var idAndLabel = clickedPage[i]
            var data = new Array();
            data["item"] = new Array();
            data.item.value = idAndLabel;
            data.item.label = idAndLabel;
            AppendSelected("ui_txtPageUrl_values", data, "pageScreen");
        }
    ...
    //    needToOpendDiv = true;
    //}
    if (ruleConditions.PageParameters != null && ruleConditions.PageParameters.length > 0) {
        $("#ui_txtQueryString").val(ruleConditions.PageParameters);
        $("#ui_chkOnQueryString").click();
        //if (ruleConditions.IsPageUrlContainsCondition == 1)
        //    $("#ui_PageUrlCheckContains").prop("checked", true);
        needToOpendDiv = true;
    }
    ...
    if (ruleConditions.AlreadyVisitedPages != null && ruleConditions.AlreadyVisitedPages.length > 0) {
        needToOpendDiv = true;
        var clickedPage2 = ruleConditions.AlreadyVisitedPages.split(",");
        for (var i = 0; i < clickedPage2.length; i++) {
            var idAndLabel2 = clickedPage2[i].replace(/\-/g, ".");
            var data = new Array();
            data["item"] = new Array();
            data.item.value = idAndLabel2;
            data.item.label = idAndLabel2;
            AppendSelectedScreen("ui_txtAlreadyVisitedPageUrls_values", data, "VisitedScreen");
        }
...
BindInteractionData = function () {
    var needToOpendDiv = false;
    if (ruleConditions.IsClickedRecentButtons != null && ruleConditions.IsClickedRecentButtons.length > 0) {
        needToOpendDiv = true;
        var clickedTags2 = ruleConditions.IsClickedRecentButtons.split(",");
        for (var i = 0; i < clickedTags2.length; i++) {
            var idAndLabel = clickedTags2[i]
            var data = new Array();
            data["item"] = new Array();
            data.item.value = idAndLabel;
            data.item.label = idAndLabel;
            AppendSelected("ui_txtRecentClickButton_values", data, "recentclkBtn");
        }
        $("#ui_chkRecentClickedButton").click();
    }
...
BehaviorData = function () {
    ...
    //}
    if ($("#ui_chkOnPageUrl").is(":checked")) {
        ruleConditions.PageUrl = GetListDataBySpanId($("#ui_txtPageUrl_values"), "id", "pageScreen_").join(",");
        ruleConditions.countOfRules = ruleConditions.countOfRules + 1;
    }
    else
        ruleConditions.PageUrl = "";

    if ($("#ui_chkOnQueryString").is(":checked")) {
        ruleConditions.PageParameters = $("#ui_txtQueryString").val();

        ruleConditions.countOfRules = ruleConditions.countOfRules + 1;
        //if ($("#ui_PageUrlCheckContains").is(":checked"))
        //    ruleConditions.PageParameters = 1;
        //else
        //    ruleConditions.PageParameters = 0;
    }
    else {
        ruleConditions.PageParameters = "";
    }
    ...
    if ($("#ui_chkAlreadyVisitedPages").is(":checked")) {
        ruleConditions.AlreadyVisitedPages = GetListDataBySpanId($("#ui_txtAlreadyVisitedPageUrls_values"), "id", "VisitedScreen_").join(",").replace(/\-/g, ".");//$("#ui_txtAlreadyVisitedPageUrls").val();
...
InteractionData = function () {
    ...
    if ($("#ui_chkAddedToCart").is(":checked")) {
        ruleConditions.AddedProductsToCart = GetListDataBySpanId($("#ui_txtAddedToCartProducts_values"), "value", "AddedToCart").join(",");
        ruleConditions.AddedProductsToCart.replace("-", "~");
        ruleConditions.countOfRules = ruleConditions.countOfRules + 1;
    }
    else {
    ...
    if ($("#hdn_domainName").val().indexOf("bestpricewholesale.co.in") > -1) {
        if ($("#ui_chkViewedNotAddedToCart").is(":checked")) {
            ...
            if ($("#ui_radViewedProductSingle").is(":checked")) {
                ...
                ruleConditions.ViewedButNotAddedProductsToCart.replace("-", "~");
            } else {
        ...
        if ($("#ui_chkDropedFromCart").is(":checked")) {
            if ($("#ui_radDroppedProductSingle").is(":checked")) {
                ...
                ruleConditions.DroppedProductsFromCart.replace("-", "~");
            } else {
    ...
    if ($("#ui_chkCustomerPurchased").is(":checked")) {
        ruleConditions.PurchasedProducts = GetListDataBySpanId($("#ui_txtCustomerPurchasedProducts_values"), "value", "Purchase").join(",");
        ruleConditions.PurchasedProducts.replace("-", "~");
        ruleConditions.countOfRules = ruleConditions.countOfRules + 1;
    }
    else {
    ...
    if ($("#ui_chkCustomerNotPurchased").is(":checked")) {
        ruleConditions.NotPurchasedProducts = GetListDataBySpanId($("#ui_txtCustomerNotPurchasedProducts_values"), "value", "NotPurchase").join(",");
        ruleConditions.NotPurchasedProducts.replace("-", "~");
        ruleConditions.countOfRules = ruleConditions.countOfRules + 1;
    }
    else {
...
/// SCreen
$("#ui_txtPageUrl").autocomplete({
    autoFocus: true,
    minLength: 0, max: 10,
    source: function (request, response) {
        GetValues(request, response, 'GetScreen')
    },
    select: function (events, selectedItem) {
        AppendSelected("ui_txtPageUrl_values", selectedItem, "pageScreen");
    },
    close: function (event, ui) {
        $(this).val("");
    }
});
$("#ui_txtAlreadyVisitedPageUrls").autocomplete({
    autoFocus: true,
    minLength: 0, max: 10,
    source: function (request, response) {
        GetValues(request, response, 'GetScreen')
    ...
    select: function (events, selectedItem) {
        AppendSelectedScreen("ui_txtAlreadyVisitedPageUrls_values", selectedItem, "VisitedScreen");
    },
    close: function (event, ui) {
        $(this).val("");
    }
...
function GetValues(request, response, method) {
    if (request != "" && method != "") {
        $.ajax({
            url: "../MobileEngagement/MobileRuleAutoSuggest",
