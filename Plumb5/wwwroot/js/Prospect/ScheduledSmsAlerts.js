...
function MaxCount() {
    $.ajax({
        url: "/Prospect/Reports/GetScheduledSmsAlertMaxCount",
        type: 'Post',
...
function GetReport() {
    ...
    $.ajax({
        url: "/Prospect/Reports/GetScheduledSmsAlertDetails",
        type: 'POST',
    ...
    if (response != undefined && response != null && response.length > 0) {
        ...
        $.each(response, function () {
            ...
            reportTableTrs +=
                ...
                "<a data-chatpop='chattransdet' " + EditContentdisplay + " class='dropdown-item chatrepvist ContributePermission' href='javascript:void(0)' onclick=\"BindScheduledSmsAlertDetails(this," + this.Id + "," + this.TemplateId + "," + this.ContactId + "," + this.IsPromotionalOrTransnational + ",'" + this.AlertScheduleDate + "')\">Edit</a>" +
                "<div class='dropdown-divider' " + EditContentdisplay + "></div>" +
                "<a data-toggle='modal' data-target='#deleterow' class='dropdown-item FullControlPermission' href='javascript:void(0)' onclick=\"DeleteScheduledSmsAlertsConfirm(" + this.Id + ");\">Delete</a>" +
                "</div>" +
                "</div>" +
                "</div>" +
                "</div>" +
...
function DeleteScheduledSmsAlertsConfirm(Id) {
    $("#deleteRowConfirmsms").attr("onclick", "DeleteScheduledSmsAlerts(" + Id + ");");
}
function DeleteScheduledSmsAlerts(Id) {
    ShowPageLoading();
    $.ajax({
        url: "/Prospect/Reports/DeleteScheduledSmsAlerts",
        type: 'POST',
        ...
        success: function (response) {
            if (response) {
                ShowSuccessMessage(GlobalErrorList.ScheduledSmsAlerts.Delete_SuccessMessage);
                GetReport();
            }
        },
        error: ShowAjaxError
