...
var IsLoadingShows = {
    ...
    IsSourceWiseLeadsCount: false, CampaignDetails: false, IsStageWiseVsLabel: false, IsSourceWiseVsLabel: false, IsStageWiseVsSourceWise: false
};

var clearTimeInterval = null;

$(document).ready(function () {
    LeadDashboardUtil.GetUserAndUserGrpList(UserId);
    LeadDashboardUtil.GetLmsStage();

    if (UserHierarchy != undefined && UserHierarchy != null && UserHierarchy != "")
        $("#ui_usernameid").html(`${UserHierarchy[0].FirstName}`)

    setTimeout(function () { GetUTCDateTimeRange(2); }, 500);
});

$("#closehierchy").click(function () {
    $("#userhierarchy").addClass("hideDiv");
    $("#dasboardwrapper").addClass("showdashboardfull");
    $("#sitemapicon").removeClass("hideDiv");
});

$("#sitemapicon").click(function () {
    $(this).addClass("hideDiv");
    $("#userhierarchy").removeClass("hideDiv");
    $("#dasboardwrapper").removeClass("showdashboardfull");
});

function CallBackWithUser(UserId, Name) {
    $(".dx-treeview-node").removeClass('dx-state-focused');
    $('[data-item-id=' + UserId + ']').addClass('dx-state-focused');
    ShowPageLoading();
    $("#ui_usernameid").html(`${Name}`);
    LeadDashboardUtil.GetUserAndUserGrpList(UserId);
...
function CallBackFunction() {
    ...
    IsLoadingShows.IsStageWiseLeadsCount = false; IsLoadingShows.IsSourceWiseLeadsCount = false;
...
function IsLoadingToHide() {
    if (IsLoadingShows.IsLeadSummary && IsLoadingShows.IsPlannedDetails && IsLoadingShows.IsLabelWiseLeadsCount && IsLoadingShows.IsStageWiseLeadsCount &&
        IsLoadingShows.IsSourceWiseLeadsCount && IsLoadingShows.CampaignDetails && IsLoadingShows.IsStageWiseVsLabel
...
var LeadDashboardUtil = {
    GetUserAndUserGrpList: function (UserId) {
        $.ajax({
            url: "/Prospect/HierarchyDashboard/GetUserHierarchyForIndividual",
