...
function MaxCount() {
    ...
    $.ajax({
        url: "/WebPush/Template/GetMaxCount",
        type: 'Post',
...
function GetReport() {
    ...
    $.ajax({
        url: "/WebPush/Template/GetTemplateList",
        type: 'Post',
    ...
    if (response != undefined && response != null) {
        ...
        $.each(response, function () {
            ...
            reportTableTrs += '<tr><td class="text-center">' +
                ...
                '<a class="dropdown-item smstemplate DesignPermission" onclick="CreateEditTemplate(' + this.Id + ',' + this.CampaignId + ',\'' + ReplaceSingleQuote(this.TemplateName) + '\',\'' + ReplaceSingleQuote(this.TemplateDescription.replace(/\n/g, '%20')) + '\');" href="javascript:void(0);">Edit</a>' +
                '<a class="dropdown-item ContributePermission" onclick="DuplicateTemplate(' + this.Id + ',' + this.CampaignId + ',\'' + ReplaceSingleQuote(this.TemplateName) + '\',\'' + ReplaceSingleQuote(this.TemplateDescription.replace(/\n/g, '%20')) + '\');" href="javascript:void(0);">Duplicate</a>' +
                '<a id="ui_a_TestWebPush_' + this.Id + '" data-title="' + this.Title + '" data-desc="' + this.MessageContent + '"  data-icon="' + this.IconImage + '" data-banner="' + this.BannerImage + '" data-btn1="' + this.Button1_Label + '"  data-btn2="' + this.Button2_Label + '" class="dropdown-item ContributePermission" onclick="CreateBrowserNotificationTestCampaign.OpenTestCampaignPopUp(' + this.Id + ',\'' + ReplaceSingleQuote(this.TemplateName) + '\');" href="javascript:void(0);">Test</a>' +
...
DeleteFormWithConform = function (Id) {
    $.ajax({
        ...
        success: function (response) {
            if (response) {
                ShowSuccessMessage(GlobalErrorList.WebpushTemplate.DeleteTemplate);
                CallBackFunction();
            }
        },
        error: ShowAjaxError
