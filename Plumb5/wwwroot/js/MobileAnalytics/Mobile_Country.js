var markerLocation = [], optiondate = '', Uniuqevistcount = [], getPageviewcount = [], getsessioncount = [],geoJSON;;
var CompareData = { Session: 0, PageView: 0, Unique: 0, New: 0, Return: 0 };

$(document).ready(function () {
    ExportFunctionName = "CountryExport";
...
function MaxCount() {
    $.ajax({
        url: "/MobileAnalytics/MobileApp/GetMobileCountryReportCount",
        type: 'Post',
...
function GetReport() {
    ...
    $.ajax({
        url: "/MobileAnalytics/MobileApp/GetMobileCountryReport",
        type: 'Post',
...
function BindReport(response) {
    ...
    if (response != undefined && response != null && response.Table != undefined && response.Table != null && response.Table.length > 0) {
        ...
        $.each(response.Table, function (m) {
            markerLocation.push({ Lat: this.Latitude, Lng: this.Longitude, Country: this.Country, PageViews: this.PageViews });
            reportTableTrs += "<tr>" +
                "<td class='text-left'><i class='flag flag-" + (this.Flag != "" && this.Flag != null ? this.Flag.trim() : "Unknown").toLowerCase() + "'></i>" + this.Country + "</td>" +
                "<td>" + this.Session + "</td>" +
                "<td>" + (this.UniqueVisits != "0" ? "<a class='ViewPermission' href='/MobileAnalytics/MobileApp/UniqueVisits?dur=0&Frm=" + FromDateTime + "&To=" + ToDateTime + "&Country=" + (this.Country != "" && this.Country && this.Country != undefined ? encodeURIComponent(this.Country.replace("'", "^")) : "unknown") + "'>" + this.UniqueVisits + "</a>" : this.UniqueVisits) + "</td>" +
...
function GetMapData() {
    $.ajax({
        url: "/MobileAnalytics/MobileApp/GetMobileCountryMapData",
        type: 'Post',
        data: JSON.stringify({ 'accountId': Plumb5AccountId, 'duration': duration, 'fromdate': FromDateTime, 'todate': ToDateTime }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
