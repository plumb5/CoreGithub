...
function BindReport(response) {
    ...
    if (MobileSessionChart) {
        MobileSessionChart.destroy();
    }
    if (response != undefined && response != null && response.Table != undefined && response.Table != null && response.Table.length > 0 && response.Table[0].Session != 0) {
        var reportTableTrs = "";
        $.each(response.Table, function () {
            ...
            reportTableTrs += "<tr>" +
                ...
                "<td>" + this.Session + "</td>" +
                "<td>" + (this.UniqueVisit != "0" ? "<a class='ViewPermission' href='/MobileAnalytics/MobileApp/UniqueVisits?dur=0&Frm=" + fromdate + "&To=" + todate + "&Cat=Visits'>" + this.UniqueVisit + "</a>" : this.UniqueVisit) + "</td> " +
                "<td>" + this.TotalVisit + "</td> " +
                "<td>" + (this.NewVisitors == '' ? "0" : this.NewVisitors) + "</td> " +
...
function BindGraph(GraphItem) {
    ...
    MobileSessionChart = new Chart(document.getElementById("ui_canvas_SessionsData"), {
        type: 'line',
        data: {
