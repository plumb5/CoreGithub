$(document).ready(function () {
    ShowPageLoading();
    ExportFunctionName = "AdSenseExportReport";
...
function MaxCount() {
    $.ajax({
        ...
        type: 'Post',
        data: JSON.stringify({ 'Action': 'GetAdSenseMaxCount', 'accountId': Plumb5AccountId, 'fromdate': FromDateTime, 'todate': ToDateTime }),
...
function GetReport() {
    ...
    $.ajax({
        ...
        type: 'Post',
        data: JSON.stringify({ 'Action': 'GetAdSenseDetails', 'accountId': Plumb5AccountId, 'fromdate': FromDateTime, 'todate': ToDateTime, 'start': OffSet, 'end': FetchNext }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: BindReport,
        error: ShowAjaxError
...
function BindReport(response) {
    ...
    if (response != undefined && response != null && response.ds.Table1 != undefined && response.ds.Table1 != null && response.ds.Table1.length > 0) {
        ...
        $.each(response.ds.Table1, function () {
            ...
            //**** For Output Cache ToDateTime *******
            UniqueVisits = (this.UniqueVisitors != "0" ? "<a class='ViewPermission' href='/Analytics/Uniques/UniqueVisits?dur=" + duration + "&Frm=" + FromDateTime + "&To=" + ToDateTime + "&AdSense=" + this.PageName.replaceAll('&', '@@@@').toString() + "'>" + this.UniqueVisitors + "</a>" : this.UniqueVisitors);
            reportTableTrs += "<tr>" +
