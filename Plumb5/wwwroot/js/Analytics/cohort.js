    ...
    var draw = function (cornelius, cohort, container) {
        ...
        function drawHeader(data) {
            ...
            for (var i = 0; i < monthLength; i++) {
                ...
                var text = config.formatHeaderLabel(i + 1 + (i == 0 ? "st" : i == 1 ? "nd" : i == 2 ? "rd" : "th") + " Month"); //i === 0 ? config.labels.people :1 2 3 4 5 6 Mb--
                th.appendChild(create('th', { text: text.substring(1) })); //, className: 'people'
            }
            return th;
        }
        ...
        function drawCells(data) {
            var fragment = document.createDocumentFragment(),
                ...
                formatPercentage = function (value, base) {
                    if (isNumber(value) && base > 0) {
                        return value; ///(value / base * 100).toFixed(2);
            ...
            for (var i = startMonth; i < data.length; i++) {
                ...
                for (var j = 0; j < data[0].length; j++) {
                    ...
                    var value = row[j],
                        cellValue = formatPercentage(value, baseValue), //j === 0 ? value : 
                        opts = {};
                    ...
                    if (!isEmpty(cellValue)) {
                        opts = {
                            ...
                            className: classNameFor(cellValue)//j === 0 ? 'people' : 
                        };
