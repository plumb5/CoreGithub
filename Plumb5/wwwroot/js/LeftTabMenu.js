...
$(document).ready(function () {
    ...
    else if (window.location.href.toLowerCase().indexOf("mobileapp/eventtracking") > -1 || window.location.href.toLowerCase().indexOf("mobileapp/alleventtrackingmobile") > -1 || window.location.href.toLowerCase().indexOf("mobileapp/eventsetting") > -1) {
    ...
    //End Mobile App
        ...
        else if (window.location.href.toLowerCase().indexOf("form/productcartdetails") > -1) {
            $('#Conv').css('display', 'block');
            $('#tabproductwishlist').css('background-color', '#f1f9df');
        }
        ...
        if (window.location.href.toLowerCase().indexOf("managescores") > -1) {
            $('#Cust').css('display', 'block');
...
function onAction() {
    ...
    if ($("#divcontent").height() > screen.height && $(window).scrollTop() > 220 && typeof (imgTop) === "undefined") {
        $("body").append('<img id="imgTop" src="/images/GoUpGreen.png" title="Go to top" onclick="GoTotop()" style="position: fixed;right: 0.5%;bottom: 35px;width: 50px;cursor:pointer" />');
        p5resetTimer();
    }
...
CallSort = function (divid) {
    ...
    $('.headerSort').click(function () {
        if ($(this).hasClass("headerSort headerSortDown")) {
    ...
    //    }
    //);
    //setTimeout(function () { $(".headerstyle > .headerSort").css({ "display": "block" }); }, 1000);
};
...
getPrintExport = function (getPageName, print, viewall, recordsperPage) {
    ...
    if (viewall == 1) {
        getViewAll = "<div style='float: left; margin-left: 5px;'>" +
            ...
            "<img src='/images/viewall.png' border='0' /><span style='width:47px; right: -0px;top: 19px;'>View All</span></a></div>";
    }
    var getPrint = "";
    if (print == 1) {
        getPrint = "<div style='float: left; margin-left: 5px;'>" +
            ...
            "<img src='/images/Print.png' border='0' /><span style='width: 40px; left: -10px; top: 30px;'>Print</span></a>" +
            "</div>";
    }
    if (recordsperPage != null && recordsperPage == 1) {///Records per page
        record = "<div class='chk' style='float:right; margin-left:0px;margin-top:2px;'><span>Records per page&nbsp;&nbsp;</span><select style='position:relative;top:1px; width:70px;' class='dropdown' id='drp_RecordsPerPage' onchange='fn_ChangeRecordsPerPage()'>" +
    ...
    "<div id='dv_shareContent' class='CustomPopUp' style='position: fixed; left: 30%; top:27%;'></div> <div id='dv_Share' //style='float: left; margin-left: 5px;'>" +
        ...
        "<img src='/images/P5_share.png' border='0' /><span style='width: 40px; left: -10px;top: 29px;'>Share</span></a>" +
        "</div>" +
        //share End
        ...
        "<img src='/images/export_icon.jpg' border='0' /><span style='width: 50px; left: -28px; top:30px;'>Export</span></a>" +
        "</div></div>";// + getViewAll;

    //return printExport;//+ getShareOptions;
    //};
...
function CheckFilter(duration) {
    ...
    if (duration == 5 && $.urlParam("P5Duration") != 0 && $('#txtDateFrom').val() == "" && $.urlParam("FromDate") != 0) {
        ...
        if ($('#chk_Maintain').is(":checked") && MaintainKey == 0) {
            chkMain = 1;
        }
        ...
        else if ((!$('#chk_Maintain').is(":checked") && ($('#chk_Maintain').is(":checked") != undefined || ChngRpp2 == 0)) && MaintainKey == 1 && duration != 0 /*&& duration != 5*/) {
            chkMain = 2;
        }
    ...
    //if (compare == 0 && window.location.href.toLowerCase().indexOf("online") == -1) {
...
function BindDate(dur, frm, to) {
    ...
    if ((window.location.href.toLowerCase().indexOf("/dashboard") > -1 || window.location.href.toLowerCase().indexOf("/audience") > -1 || window.location.href.toLowerCase().indexOf("/MobileApp")) && dur != 1 && dur != 5 && dur != 10) {
        var newdate = a.setDate(a.getDate() - 1);
        b = newdate = new Date(newdate);
        switch (dur) {
            case 2:
        ...
        switch (dur) {
            ...
            case 5:
                a = new Date(frm);
                b = new Date(to);
                window.lblDate.innerHTML = a.getDate() + ' ' + GetMonthName(a.getMonth() + 1) + ' ' + a.getFullYear() + ' - ' + b.getDate() + ' ' + GetMonthName(b.getMonth() + 1) + ' ' + b.getFullYear();
                startdate = (a.getMonth() + 1) + '/' + a.getDate() + '/' + a.getFullYear() + " 00:00:00.000";
                enddate = (b.getMonth() + 1) + '/' + b.getDate() + '/' + b.getFullYear() + " 23:59:59.000";
                if ($("#chk_Maintain").is(":checked")) {
...
function ConvertDateTimeToUTC(dateString) {
    ...
    if (dateString.indexOf("/") > -1) {
        ...
        if (navigator.userAgent.indexOf("Firefox") != -1 || navigator.userAgent.indexOf("Mozilla") != -1) {
            if (dateString.indexOf("/") > -1) {
                ...
                var time = dateString.split('/')[2].split(' ')[1].split(':')
                JavaScriptDateTime = new Date(year, date[0] - 1, date[1], time[0], time[1], time[2]);
            }
        }
        else {
    ...
    if (navigator.userAgent.indexOf("Firefox") != -1 || navigator.userAgent.indexOf("Mozilla") != -1) {
        console.log(JavaScriptDateTime);
        return JavaScriptDateTime;
    }
    var UtcDateTime = new Date(JavaScriptDateTime.getTime() + (JavaScriptDateTime.getTimezoneOffset() * 60 * 1000));
...
function ConvertUTCDateTimeToLocal(dateString) {
    ...
    var LocalDateTime = new Date(JavaScriptDateTime.getTime() - (JavaScriptDateTime.getTimezoneOffset() * 60 * 1000));
...
function getSharePopup() {
    var ShareDiv = "<div class='newTitle'><span id='sp_shareTtl'>Share Report</span>" +
        ...
        "style='cursor: pointer; float: right' src='/images/Lofinclose.jpg' />" +
        "</div>" +
