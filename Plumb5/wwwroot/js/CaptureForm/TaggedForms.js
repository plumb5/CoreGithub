var EmbeddedFormOrPopUpFormOrTaggedForm = 'TaggedForm', IsBannerOrForm = false, FormId = 0, VisitorType = 0, reportType = "By Forms", FormResponses = [];
...
$(".frmrepsubmenu").click(function () {
    ...
    $('#ui_drpdwn_AllTaggedForms').empty();
    $('#ui_drpdwn_AllTaggedForms').append("<option label='Search Forms' value='0'>Search Forms</option>");
    CallBackFunction();
});
...
$('#ui_drpdwn_AllTaggedForms').on('change', function () {
    formResponses.FormId = FormId = $('#ui_drpdwn_AllTaggedForms').val();
    CallBackFunction();
});

//************************************Get all tagged forms**********************************//
function GetAllForms() {
    $.ajax({
        url: "/CaptureForm/CommonDetailsForForms/GetFormsListBasedOnType",
        type: 'POST',
        data: JSON.stringify({ 'AdsId': Plumb5AccountId, 'fromDateTime': "", 'toDateTime': "", 'EmbeddedFormOrPopUpFormOrTaggedForm': EmbeddedFormOrPopUpFormOrTaggedForm }),
        ...
        success: function (response) {
            ...
            $.each(response, function () {
                ...
                document.getElementById("ui_drpdwn_AllTaggedForms").options.add(optlist);
            });
        },
        error: ShowAjaxError
    });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (TotalRowCount > 0) {
                ...
                SetNoRecordContent('tbl-forms', 5, 'ui_tblReportData_TaggedForms');
                ShowExportDiv(false);
                ShowPagingDiv(false);
                HidePageLoading();
            }
...
function BindFormReport(response) {
    ...
    if (response !== undefined && response !== null && response.length > 0) {
        ...
        $("#ui_tblReportData_TaggedForms").html(reportTableTrs);
    }
    else {
        ShowExportDiv(false);
        ShowPagingDiv(false);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (TotalRowCount > 0) {
                ...
                SetNoRecordContent('tbl-response', 4, 'ui_tblReportData_TaggedResponse');
                ShowExportDiv(false);
                ShowPagingDiv(false);
                HidePageLoading();
            }
...
function BindResponseReport(response) {
    ...
    if (response !== undefined && response !== null && response.length > 0) {
        ...
        $("#ui_tblReportData_TaggedResponse").html(reportTableTrs);
    }
    else {
        ShowExportDiv(false);
        ShowPagingDiv(false);
...
$('#ui_drpdwn_CustomReportsByForm').on('change', function () {
    ...
    $('#ui_drpdwn_AllTaggedForms').val(formResponses.FormId);
});
//************************************Custom Report**********************************//
...
$('#ui_drpdwn_AllTaggedForms,#ui_drpdwn_CustomReportsByForm,#ui_drpdwn_FilterByFields').select2({
...
$('#eventitemstitle').on('change', function () {
    ...
    if ((90 - $("#selTitleTxt").val().length) > 15 && this.value != "Select") {
        $("#selTitleTxt").val($("#selTitleTxt").val() + " {{*[" + eventname + "]~[" + this.value + "]~[TOP1.DSC]~[fallbackdata]*}}");
        var str = PreviewSupport($("#selTitleTxt").val());
...
function AppendCustomField(MessageBody) {
    ...
    for (var i = 0; i < customeventFieldList.length; i++) {
        if (MessageBody.toLowerCase().indexOf("{{*[" + customeventFieldList[i][0].toLowerCase() + "]") > -1) {
            var regExpevents = new RegExp("{{[" + customeventFieldList[i][0] + "]", "ig");
            MessageBody = MessageBody.replace(regExpevents, "{{*[" + customeventFieldList[i][0] + "]");
            for (var j = 1; j < customeventFieldList[i].length; j++) {
                if (MessageBody.toLowerCase().indexOf("]~[" + customeventFieldList[i][j].toLowerCase() + "]~[") > -1) {
                    var regExp = new RegExp("\\]\\~\\[" + customeventFieldList[i][j] + "\\]\\~\\[", "ig");  // regex pattern string
                    MessageBody = MessageBody.replace(regExp, "]~[EventData" + (j) + "]~[");
                }
...
$('#eventitemsmessage').on('change', function () {
    ...
    if ((160 - $("#selDescTxt").val().length) > 15 && this.value != "Select") {
        $("#selDescTxt").val($("#selDescTxt").val() + " {{*[" + eventname + "]~[" + this.value + "]~[TOP1.DSC]~[fallbackdata]*}}");
        var str = PreviewSupport($("#selDescTxt").val());
...
$('#eventitemsmessage2').on('change', function () {
    //messagetextboxvalue = " { { [" + eventname + "][" + this.value + "][TOP1.DSC]~[fallbackdata] } }";
    if ((160 - $("#ui_txtContent").val().length) > 15 && this.value != "Select") {
        $("#ui_txtContent").val($("#ui_txtContent").val() + " {{*[" + eventname + "]~[" + this.value + "]~[TOP1.DSC]~[fallbackdata]*}}");
        var str = PreviewSupport($("#selDescTxt").val());
