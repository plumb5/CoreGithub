var FormDetails = { Id: 0, FormType: 0, FormIdentifier: "", Heading: "", FormCampaignId: 0, FormStatus: null, OnPageOrInPage: null, IsWebOrMobileForm: null, AppearOnLoadOnExitOnScroll: 0, IsOtpForm: false, OTPFormId: 0, ClientCampaignIdentifier: "", CampaignIdentifier: 0, CampaignIdentifierName: "", SubHeading: "", IsClickToCallForm: false, IsVerifiedEmail: false, IsAutoWhatsApp: false };
var FormStatus;
var IsOtpForm = false, OTPFormId = 0;
var formBasicDetails, formDesign, formFields, formBannerList, formFieldsBindingDetails;

$(document).ready(function () {
    ExportFunctionName = "EmbeddedFormsExport";
...
function MaxCount() {
    ...
    $.ajax({
        url: "/CaptureForm/ManageEmbeddedForms/GetMaxCount",
        type: 'Post',
...
function GetReport() {
    ...
    $.ajax({
        url: "/CaptureForm/ManageEmbeddedForms/GetAllDetails",
        type: 'POST',
    ...
    if (response != undefined && response != null && response.length > 0) {
        ...
        $.each(response, function () {
            ...
            reportTableTrs +=
                ...
                "<input type='checkbox' class='custom-control-input selembChk' id='embedd_" + this.Id + "' value='" + this.Id + "' name='embeddedfrm'>" +
                "<label class='custom-control-label' for='embedd_" + this.Id + "'></label>" +
                "</div>" +
                "</td>" +
                "<td>" +
                "<div class='groupnamewrap'>" +
                "<div class='frmnametxt' id='embeddform_" + this.Id + "' title='" + this.FormIdentifier + "'>" +
                "<i class='griddragicn ContributePermission'></i><div class='frmidntwrp'><span class='wordbreak'>" + formidentifername + "</span>" +
                ...
                "<a id='dv_duplicatePopUpForm' data-toggle='modal' data-target='#duplicateconfirmation' data-PopUpDuplicateformid=" + this.Id + " data-grouptype='groupduplicate' class='dropdown-item ContributePermission' href='javascript:void(0)'>Duplicate</a>" +
                ...
                "<a class='dropdown-item embfrmlist ContributePermission' href='javascript:void(0)' onclick='GetScripDetails(" + this.Id + "," + this.FormType + ",\"" + this.FormIdentifier + "\");'>Get In-page Tag</a>" +
...
$("#SearchByIdentifierName").click(function () {
    ...
    if ($.trim($("#ui_txtSearchBy").val()).length == 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.FormIdentiferSearch_error);
...
ChangeStatus = function (Id, status) {
    ...
    $.ajax({
        url: "/CaptureForm/ManageEmbeddedForms/ToogleStatus",
        type: 'POST',
        ...
        success: function (response) {
            if (response) {
                ...
                ShowSuccessMessage(GlobalErrorList.ManageEmbeddedForm.ToggleSuccessStatus);
            }
            else {
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.ToggleFailureStatus);
            }
            FormDetails.Id = 0;
            FormDetails.FormStatus = null;
        },
...
$("#deleteRowConfirm").click(function () {
    ...
    $.ajax({
        url: "/CaptureForm/ManageEmbeddedForms/Delete",
        type: 'POST',
        ...
        success: function (response) {
            if (response) {
                ...
                PagingPrevNext(OffSet, CurrentRowCount, TotalRowCount);
                ShowSuccessMessage(GlobalErrorList.ManageEmbeddedForm.DeleteSuccessStatus);
                BindOTPForms();
                ...
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.DeleteFailureStatus);
            }
        },
        error: ShowAjaxError
    });
...
function CopyFormDetails(Id) {
    ShowPageLoading();
    $.ajax({
        url: "/CaptureForm/ManageEmbeddedForms/CopyFormDetails",
        type: 'Post',
        ...
        success: function (response) {
            if (response > 0) {
                ShowSuccessMessage(GlobalErrorList.ManageEmbeddedForm.DuplicateFormSuccessStatus);
                CallBackFunction();
            }
            else {
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.DuplicateFormFailureStatus);
            }
            HidePageLoading();
        },
        error: ShowAjaxError
...
var selembbedchkcount;
$(document).on('click', ".selembChk", function () {
    selembbedchkcount = $('.selembChk').filter(':checked').length;
    if (selembbedchkcount == 0) {
        $(".subdivWrap").removeClass('showDiv');
    }
    else if (selembbedchkcount > 4) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.FormSelectionErrorMessage);
    }
    else {
        $(".subdivWrap").addClass('showDiv');
        $(".checkedCount").html(selembbedchkcount);
    }
});

$(".addmultivariatefrm").click(function () {
    if ($('.selembChk').filter(':checked').length <= 4) {
        $("#ui_MultiConditionalmobileno").prop("checked", true);
        $("#ui_ctcnoMultiConditional").prop("checked", true);
        $("#dvMultiConditionalForm .popuptitle h6").html("Create Multi-Variate Form");
        $("#dvMultiConditionalForm").removeClass("hideDiv");
        $("#dvCampaignSettings").addClass("hideDiv");
        $("#dvPreview").addClass("hideDiv");
        $("#dvFormScriptDetails").addClass("hideDiv");
        $(".popupItem").removeClass("popup-tbl");
        BindSelectedFormsToTable();

        var today = new Date();
        var strYear = (today.getMonth() + 1).toString() + today.getDate().toString() + today.getMilliseconds().toString();
...
function GetScripDetails(formId, FormType, FormIdentifier) {
    ShowPageLoading();
    $("#dvFormScriptDetails").removeClass("hideDiv");
    $("#dvCampaignSettings,#dvMultiConditionalForm,#dvPreview").addClass("hideDiv");
    $(".popuptitle h6").html("Form Script Details" + "(" + FormIdentifier + ")");
    $("#copydivtag").val('').val('<div id="dvPlumb5InPageScript' + formId + '" plumb5="Plumb5InPageForms" plumb5formidentifier="' + formId + '"></div>');
    HidePageLoading();
}

$(".divtagcopybtn").click(function () {
    var copyText = document.getElementById("copydivtag");
    ...
    $(".copiedtxtwrp").css("display", "block").html("Div Tag is Copied!");

    setTimeout(function () {
        $(".copiedtxtwrp").css("display", "none");
    }, 3000);
});
...
function ManageCampaignDetails(Id) {
    ...
    $("#dvMultiConditionalForm").addClass("hideDiv");
    $("#dvPreview").addClass("hideDiv");
    $("#dvFormScriptDetails").addClass("hideDiv");
    $(".popuptitle h6").html("Settings");
    ...
    $.ajax({
        ...
        success: function (response) {
            if (!response.Status) {
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.FormCampaignIdentiferErrorDetails);
                HidePageLoading();
            }
                ...
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.SessionExpired);
                HidePageLoading();
            }
            else {
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.FormCampaignIdentiferErrorDetails);
...
$("#txtCampaignIdentifier").autocomplete({
    ...
    select: function (events, selectedItem) {
        if ($('#ui_txtCampaignIdentifier_values').children('div').length < 1)
            ...
            ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.CampaignIdentifierErrorItem);
    },
...
function AppendSelected(appendTo, data, fieldId) {
    ...
    if (isElementIsNotAdded)
        ...
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.ItemAlreadyAdded);
}
...
$("#btnSaveCampaignDetails").click(function () {
    ...
    if (Id > 0) {
        $.ajax({
            ...
            type: 'POST',
            data: JSON.stringify({ 'Id': Id, 'ClientCampaignIdentifier': ClientCampaignIdentifier, 'CamapignId': CampaignIdentifer, 'CampaignIdentiferName': CampaignIdentiferName, 'IsOtpForm': false, 'OTPFormId': 0, 'IsWebOrMobileForm': IsWebOrMobileForm, 'OTPGenerationLimits': 0, 'OTPPageRestrictions': false, 'IsClickToCallForm': IsClickToCallForm, 'IsVerifiedEmail': IsVerifiedEmail, 'IsAutoWhatsApp': IsAutoWhatsApp, 'BlockEmailIds': BlockEmailIds }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                if (response) {
                    ...
                    ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.CampaignIdentifierFailureStatus);
                    HidePageLoading();
                }
            },
            error: ShowAjaxError
...
ValidateCampaignDetails = function () {
    ...
    if ($.trim($("#txtClientCampaignIdentifier").val()).length == 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EnterClientCampaignIdentifer);
        return false;
    }

    if ($('#ui_txtCampaignIdentifier_values').children('div').length <= 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EnterCampaignIdentifer);
        return false;
    }

    if ($('#ui_OTPyes').is(":checked") && $("#ddlOtpform").get(0).selectedIndex <= 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.SelectOTPForm);
        return false;
    }
...
function BindOTPForms() {
    ...
    $.ajax({
        ...
        type: 'POST',
        data: JSON.stringify({ 'FormType': "EmbeddedForm" }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
...
$('.sorted_table').sortable({
    ...
    onDrop: function ($item, container, _super) {
        ...
        $.ajax({
            url: "/CaptureForm/ManageEmbeddedForms/ChangePriority",
            type: 'POST',
...
function BindSelectedFormsToTable() {
    var FormIds = GetSelectedFormIds();

    if (FormIds != null && FormIds.length > 0) {
        BindFormsListTable(FormIds);
    }
}
...
GetSelectedFormIds = function () {
    ...
    $("input:checkbox:checked").map(function () {
        formListId.push($(this).val());
    });
    return formListId;
};

$("#txtMultiConditionalCampaignIdentifier").autocomplete({
    autoFocus: true,
    minLength: 1, max: 10,
    source: function (request, response) {
        var value = $("#txtMultiConditionalCampaignIdentifier").val();
        $.ajax({
            url: "/CaptureForm/CommonDetailsForForms/GetCampaignIdentifierDetails",
            data: JSON.stringify({ 'value': value }),
            dataType: "json",
    ...
    select: function (events, selectedItem) {
        if ($('#ui_txtMultiConditionalCampaignIdentifier_values').children('div').length < 1)
            AppendSelected("ui_txtMultiConditionalCampaignIdentifier_values", selectedItem, "MultiConditionalCampaignIdentifier");
        else
            ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.CampaignIdentifierErrorItem);
    },
    close: function (event, ui) {
...
$("#btnSaveMultiConditionalForm").click(function () {
    ShowPageLoading();

    if (!ValidateMultiConditionalForm()) {
        HidePageLoading();
        return;
    }

    var Id = 0;
    FormDetails.ClientCampaignIdentifier = $.trim($("#txtMultiConditionalClientCampaignIdentifier").val());
    FormDetails.CampaignIdentifierName = GetListDataBySpanId($("#ui_txtMultiConditionalCampaignIdentifier_values"), "value", "").join(",");
    FormDetails.CampaignIdentifier = GetListDataBySpanId($("#ui_txtMultiConditionalCampaignIdentifier_values"), "CampaignId", "").join(",");
    FormDetails.IsWebOrMobileForm = false;
    FormDetails.IsClickToCallForm = false;

    if ($("input[name='IsMobileFormMultiConditional']:checked").val() == "1") {
        FormDetails.IsWebOrMobileForm = true;
    }

    if ($("input[name='IsCTCFormMultiConditional']:checked").val() == "1") {
        FormDetails.IsClickToCallForm = true;
    }

    FormDetails.FormIdentifier = CleanText($("#ui_txtFormIdentifier").val());
    FormDetails.Heading = CleanText($("#ui_txtFormTitle").val());
    FormDetails.SubHeading = CleanText($("#ui_txtFormDescription").val());
    FormDetails.FormType = 19;
    FormDetails.PageContentForEmbed = "";
    FormDetails.OnPageOrInPage = false;
    FormDetails.FormStatus = false;

    if ($("#btnSaveMultiConditionalForm").attr("FormId") != undefined) {
        FormDetails.Id = $("#btnSaveMultiConditionalForm").attr("FormId");

        FormDetails.FormStatus = $("#btnSaveMultiConditionalForm").attr("Formstatus");

        var formids = [];

        $("#tbodyMultiConditionalForms tr").each(function () {
            formids.push(this.id);
        });

        for (var i = 0; i < formids.length; i++) {
            if ($("#" + formids[i]).attr('value') != undefined && $("#" + formids[i]).attr('value') != null && $("#" + formids[i]).attr('value') != "") {
                FormDetails.PageContentForEmbed += "@$" + $("#" + formids[i]).attr('value');
            }
        }
    }
    else {
        ...
        for (var i = 0; i < SelectedFormIds.length; i++) {
            FormDetails.PageContentForEmbed += "@$" + SelectedFormIds[i];
        }
    }

    if (FormDetails.PageContentForEmbed.length > 0)
        FormDetails.PageContentForEmbed = FormDetails.PageContentForEmbed.substring(2);

    $.ajax({
        url: "/CaptureForm/MultiConditionalForm/SaveOrUpdateMultiConditionalForm",
        type: 'POST',
        data: JSON.stringify({ 'accountId': Plumb5AccountId, 'formdetails': FormDetails }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            if (response.Status) {
                ...
                //ShowScript(formId, true);
                ShowSuccessMessage(GlobalErrorList.ManageEmbeddedForm.FormSaveSuccessStatus);
                setTimeout(function () { $("#dvMultiConditionalForm").addClass("hideDiv"); }, 2000);
                $('input:checkbox').removeAttr('checked');
                $(".subdivWrap").removeClass('showDiv');
                HidePageLoading();
            }
            else if (!response.Status) {
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.FormSaveErrorStatus);
                HidePageLoading();
            }
        },
        error: ShowAjaxError
...
ValidateMultiConditionalForm = function () {

    if ($.trim($("#txtMultiConditionalClientCampaignIdentifier").val()).length == 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EnterClientCampaignIdentifer);
        return false;
    }

    if ($('#ui_txtMultiConditionalCampaignIdentifier_values').children('div').length <= 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EnterCampaignIdentifer);
        return false;
    }

    if (CleanText($("#ui_txtFormIdentifier").val()).length == 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EnterFormIdentifier);
        return false;
    }

    if (CleanText($("#ui_txtFormTitle").val()).length == 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EnterFormTitle);
        return false;
    }

    if (CleanText($("#ui_txtFormDescription").val()).length == 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EnterFormDescription);
        return false;
    }

    if ($("#tbodyMultiConditionalForms > tr").length <= 0) {
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.EmptyFormSelectionErrorMessage);
        return false;
    }
...
function BindMultiConditionalFormDetails(Id) {
    ShowPageLoading();
    $("#ui_MultiConditionalmobileno").prop("checked", true);
    $("#ui_ctcnoMultiConditional").prop("checked", true);
    $("#dvCampaignSettings").addClass("hideDiv");
    $("#dvMultiConditionalForm").removeClass("hideDiv");
    $(".popuptitle h6").html("Update Multi-Conditional Form");

    $.ajax({
        url: "/CaptureForm/MultiConditionalForm/GetFormDetails",
        data: JSON.stringify({ 'accountId': Plumb5AccountId, 'FormId': Id }),
        dataType: "json",
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataFilter: function (data) { return data; },
        success: function (response) {
            if (!response.saveForm.Status) {
                ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.FormBindingErrorDetails);
                HidePageLoading();
            }
            else if (response.saveForm.Status) {
                formDetails = response.saveForm.savedFormdetails;
                if (formDetails != null && formDetails != "") {
                    ...
                    // $(".embedformidentifer").attr('title', formDetails.FormIdentifier).html(unescape(FormIdentifier));
                    $('#ui_txtMultiConditionalCampaignIdentifier_values').children('div').remove();
                    $("#txtMultiConditionalClientCampaignIdentifier").val("");
                    $("#btnSaveMultiConditionalForm").removeAttr("FormId").attr("FormId", Id);
                    $("#btnSaveMultiConditionalForm").removeAttr("Formstatus").attr("Formstatus", formDetails.FormStatus);

                    if (formDetails.ClientCampaignIdentifier != null && formDetails.ClientCampaignIdentifier != "" && formDetails.CampaignIdentifier != null && formDetails.CampaignIdentifier != "") {
                        $("#txtMultiConditionalClientCampaignIdentifier").val(formDetails.ClientCampaignIdentifier);
                        var data = new Array();
                        data["item"] = new Array();
                        data.item.value = formDetails.CampaignIdentifierName;
                        data.item.label = formDetails.CampaignIdentifierName;
                        data.item.assignedValue = formDetails.CampaignIdentifier;
                        AppendSelected("ui_txtMultiConditionalCampaignIdentifier_values", data, "CampaignIdentifier");

                        if (formDetails.IsWebOrMobileForm == 1) {
                            $("#ui_MultiConditionalmobileyes").prop("checked", true);
                        }
                    }

                    BindMultiConditionalFormsListTable(response.multicondformlist);
                    HidePageLoading();
                }
            }
            else if (resultValue == -10) {
...
BindFormsListTable = function (FormIds) {
    if (FormIds != null && FormIds.length > 0) {

        $("#tbodyMultiConditionalForms").empty();

        var tdContent = "";
        var formidentifername = "";

        for (var i = 0; i < FormIds.length; i++) {
            if ($("#embeddform_" + FormIds[i]).attr('title') != undefined && $("#embeddform_" + FormIds[i]).attr('title') != null && $("#embeddform_" + FormIds[i]).attr('title') != "") {
                formidentifername = $("#embeddform_" + FormIds[i]).attr('title');
                ...
                tdContent +=
                    "<tr id='ui_divmulti_" + FormIds[i] + "'>" +
                    " <td>" + formidentifername + "</td>" +
                    "<td class='text-center multidelwrp'><i id='dv_deleteMultiConditonalForm' data-toggle='modal' data-target='#deleteMutiConditonalforms' data-MultiConditonalFormId=" + FormIds[i] + " class='icon ion-trash-b'></i></td>" +
                    " </tr>";
            }
        }

        $("#tbodyMultiConditionalForms").html(tdContent);
    }
    HidePageLoading();
};

//BindMultiConditionalFormsListTable = function (FormIds) {
//    if (FormIds != null && FormIds.length > 0) {
...
//            var sample = JSLINQ(formList).Where(function () {
//                return (this.Id == FormIds[i]);
//            });

//            if (sample.items[0] != null && sample.items[0].FormIdentifier != null) {
//                formidentifername = sample.items[0].FormIdentifier;
...
//                tdContent +=
//                    "<tr id='ui_divmulti_" + FormIds[i] + "' value=" + FormIds[i] + ">" +
//                    " <td>" + formidentifername + "</td>" +
//                    "<td class='text-center multidelwrp'><i id='dv_deleteMultiConditonalForm' data-toggle='modal' data-target='#deleteMutiConditonalforms' data-MultiConditonalFormId=" + FormIds[i] + " class='icon ion-trash-b'></i></td>" +
//                    " </tr>";
//            }
...
BindMultiConditionalFormsListTable = function (MultiConditionFormIdList) {
    if (MultiConditionFormIdList != null && MultiConditionFormIdList.length > 0) {
        ...
        for (var i = 0; i < MultiConditionFormIdList.length; i++) {

            if (MultiConditionFormIdList[i].FormIdentifier != null && MultiConditionFormIdList[i].FormIdentifier != null) {
                formidentifername = MultiConditionFormIdList[i].FormIdentifier;
                ...
                tdContent +=
                    "<tr id='ui_divmulti_" + MultiConditionFormIdList[i].Id + "' value=" + MultiConditionFormIdList[i].Id + ">" +
                    " <td>" + formidentifername + "</td>" +
                    "<td class='text-center multidelwrp'><i id='dv_deleteMultiConditonalForm' data-toggle='modal' data-target='#deleteMutiConditonalforms' data-MultiConditonalFormId=" + MultiConditionFormIdList[i].Id + " class='icon ion-trash-b'></i></td>" +
                    " </tr>";
            }
        }
...
var MutliFormId = 0;
$(document).on('click', "#dv_deleteMultiConditonalForm", function () {
    MutliFormId = parseInt($(this).attr("data-MultiConditonalFormId"));

});

$("#deleteRowConfirmForMultiCondForm").click(function () {
    $("#ui_divmulti_" + MutliFormId).remove();
    $("#embedd_" + MutliFormId).prop("checked", false);
});

function EditDetails(FormType, FormId) {
    if (FormType == 19) {
        BindMultiConditionalFormDetails(FormId);
    }
    else {
        window.location.href = "/CaptureForm/Create/EmbeddedForm?FormType=" + FormType + "&FormId=" + FormId + "";
    }
}

$("#ui_aCreateEmbeddedForm").click(function () {
    ShowPageLoading();
    $.ajax({
        ...
        success: function (response) {
            if (response != undefined && response != null && response.Status != undefined && response.Status != null && response.Status) {
                window.location.href = "/CaptureForm/Create?FormType=EmbeddedForm";
            }
            else {
...
function ManagePreviewDetails(formId, FormType, FormIdentifier) {
    ...
    if (FormType != 19) {
        ...
        ShowErrorMessage(GlobalErrorList.ManageEmbeddedForm.MultiConditionalFormPreviewError);
        HidePageLoading();
    }
}
...
function bannerImageAppend(iframeid) {
    var IframeElement = document.getElementById(iframeid);
    var innerDoc = IframeElement.contentDocument || IframeElement.contentWindow.document;
