var EmbeddedFormOrPopUpFormOrTaggedForm = 'PopUpForm', IsBannerOrForm = false, FormId = 0, VisitorType = 0, reportType = "By Forms", FormResponses = [];
...
$(".frmrepsubmenu").click(function () {
    ...
    $('#ui_drpdwn_AllPopUpForms').empty();
    $('#ui_drpdwn_AllPopUpForms').append("<option label='All Forms' value='0'>All Forms</option>");
    CallBackFunction();
});
...
$('#ui_drpdwn_AllPopUpForms').on('change', function () {
    ShowPageLoading();

    formResponses.FormId = FormId = $('#ui_drpdwn_AllPopUpForms').val();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function () {
                ...
                document.getElementById("ui_drpdwn_AllPopUpForms").options.add(optlist);
            });
        },
        error: ShowAjaxError
    });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (TotalRowCount > 0) {
                ...
                SetNoRecordContent('tbl-forms', 5, 'ui_tblReportData_PopUpForms');
                ShowExportDiv(false);
                ShowPagingDiv(false);
                HidePageLoading();
            }
...
function BindFormReport(response) {
    ...
    if (response !== undefined && response !== null && response.length > 0) {
        ...
        $("#ui_tblReportData_PopUpForms").html(reportTableTrs);
    }
    else {
        ShowExportDiv(false);
        ShowPagingDiv(false);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (TotalRowCount > 0) {
                ...
                SetNoRecordContent('tbl-response', 4, 'ui_tblReportData_PopUpResponse');
                ShowExportDiv(false);
                ShowPagingDiv(false);
                HidePageLoading();
            }
...
function BindResponseReport(response) {
    ...
    if (response !== undefined && response !== null && response.length > 0) {
        ...
        $("#ui_tblReportData_PopUpResponse").html(reportTableTrs);
    }
    else {
        ShowExportDiv(false);
        ShowPagingDiv(false);
...
function BindMoreResponseData(Id, allFormFields) {
    ...
    tdContent += "<tr>" +
        ...
        "</tr>";
    if (allFormFields.Data !== null && allFormFields.Data !== undefined && allFormFields.Data.formFields.length > 0) {
        for (var l = 0; l < allFormFields.Data.formFields.length; l++) {
            var value = responsedetails[0]["Field" + (l + 1)] !== null ? responsedetails[0]["Field" + (l + 1)] : "NA";
            tdContent += "<tr>" +
                "<td class='text-left td-icon'>" + allFormFields.Data.formFields[l].Name + "</td>" +
...
$('#ui_drpdwn_CustomReportsByForm').on('change', function () {
    ...
    $('#ui_drpdwn_AllPopUpForms').val(formResponses.FormId);
});
//************************************Custom Report**********************************//

$("#ui_drpdwn_AllPopUpForms,#ui_drpdwn_CustomReportsByForm,#ui_drpdwn_FilterByFields").select2({
