...
var FullCalendar = (function (exports) {
    ...
    */
    ...
    function reduceDateProfile(currentDateProfile, action, currentDate, dateProfileGenerator) {
        ...
        var dp; PrevMonthCliked = false; NextMonthClicked = false;
        switch (action.type) {
            case 'CHANGE_VIEW_TYPE':
            ...
            case 'PREV':
                ...
                if (currentDateProfile.currentRangeUnit != undefined && currentDateProfile.currentRangeUnit.toLowerCase() == "month") {
                    PrevMonthCliked = true;
                }             
                SelectedDate = currentDate;
                if (dp.isValid) {
                    return dp;
    ...
    */
    ...
    function buildRequestParams(meta, range, context) {
        ...
        if (typeof meta.extraParams === 'function') {
            ...
            // Date time changing on prev/next month changing
            if (PrevMonthCliked) {
                let prevDate = new Date(SelectedDate);
                prevDate = new Date(prevDate.getFullYear(), prevDate.getMonth() - 1);
                meta.extraParams.fromDateTime = GetUTCStartDateTime(prevDate);
                meta.extraParams.toDateTime = GetUTCEndDateTime(prevDate);
            }
            else if (NextMonthClicked) {
                let nextDate = new Date(SelectedDate);
                nextDate = new Date(nextDate.getFullYear(), nextDate.getMonth() + 1);
                meta.extraParams.fromDateTime = GetUTCStartDateTime(nextDate);
                meta.extraParams.toDateTime = GetUTCEndDateTime(nextDate);
            }
            else {
                let todaydate = new Date(SelectedDate);
                meta.extraParams.fromDateTime = GetUTCStartDateTime(todaydate);
                meta.extraParams.toDateTime = GetUTCEndDateTime(todaydate);
            }
            customRequestParams = meta.extraParams || {};
        }
