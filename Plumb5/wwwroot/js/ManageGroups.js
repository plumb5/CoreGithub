...
function MaxCount() {
    ...
    $.ajax({
        url: "../Prospect/Groups/GetMaxCount",
        type: 'POST',
...
function CreateTable(numberRowsCount) {
    ...
    $.ajax({
        url: "../Prospect/Groups/BindGroupsContact",
        type: 'Post',
...
function GeneralBinding(data, isAddOrUpdate) {
    ...
    if (groupDescription.length > 25) {
        groupDescription = groupDescription.substring(0, 25) + "..";
    }
    ...
    tdContent += "<td style='text-align: right;width: 10%;'><a  href='../Prospect/Leads?GroupId=" + data.Id + "&year=4'>" + Total + "</a></td>";
        ...
        if (mailGroupDetailsObject.Id > 0) {
            ...
            for (var i = 0; i < GroupDetails.length; i++) {
                if (GroupDetails[i].Id == mailGroupDetailsObject.Id) {
                    ...
                    mailGroupDetailsObject.Total = GroupDetails[i].Total;
                    break;
                }
            }
